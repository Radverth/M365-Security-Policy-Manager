...
try {
  ...
  // ── Shared full-backup helpers (used by the disk export and the IT Glue
  //    upload — both produce the same artifacts: a restorable JSON manifest,
  //    one JSON file per policy, a fully-detailed PDF, and a README) ─────────

  const backupSafeName = s => String(s || '')
    .replace(/[^a-zA-Z0-9 \-_.]/g, '')
    .trim()
    .replace(/\s+/g, '_')
    .slice(0, 60) || 'unnamed'

  // Best-effort tenant display name for folder/zip names and the PDF header
  const resolveTenantName = async ({ account, tenantId }) => {
    let tenant = null
    if (psSession.alive) {
      try {
        const lines = []
  ...
  if ($org -and $org.value -and $org.value.Count -gt 0) { Write-Output "TENANT_NAME:$($org.value[0].displayName)" }
    ...
    return tenant || account || tenantId || 'Tenant'
  }

  // One JSON file per policy (named after the policy, de-duplicated)
  const buildPolicyFileEntries = (policies) => {
    const usedNames = new Set()
    return policies.map(p => {
      const base = backupSafeName(p.DisplayName || p.displayName || p.Id || p.id || 'policy')
      let name = base, n = 2
      while (usedNames.has(name.toLowerCase())) name = `${base}_${n++}`
      usedNames.add(name.toLowerCase())
      return { filename: `${name}.json`, content: JSON.stringify(p, null, 2) }
    })
  }

  // PDF with the full configuration of every policy, rendered off-screen
  const renderBackupPdf = async (tenant, policies, nameMap, now) => {
  ...
  // README explaining the contents and how to restore. storedNote describes
  // where this particular copy of the backup lives.
  const buildBackupReadme = ({ tenant, tenantId, account, now, policyCount, storedNote }) => `M365 Security Policy Manager — Policy Backup
=============================================
...
Contents
--------
policies-backup.json    Complete backup of all ${policyCount} Conditional Access
...
`
  ...
  // timestamped backup folder is created with all the artifacts above.
  ...
  ipcMain.handle('backup:export', async (_, { policies: pols, tenantId, account, orgName, nameMap = {} } = {}) => {
    try {
      ...
      const tenant = orgName || await resolveTenantName({ account, tenantId })
      ...
      const folder = path.join(filePaths[0], `PolicyBackup_${backupSafeName(tenant)}_${stamp}`)
      ...
      // 2. One JSON file per policy
      for (const entry of buildPolicyFileEntries(policies)) {
        fs.writeFileSync(path.join(folder, 'policies', entry.filename), entry.content, 'utf-8')
      }

      // 3. PDF with the full configuration of every policy
      const pdfBuffer = await renderBackupPdf(tenant, policies, nameMap, now)
      fs.writeFileSync(path.join(folder, `PolicyBackup_${backupSafeName(tenant)}_${stamp}.pdf`), pdfBuffer)

      // 4. README explaining the contents and how to restore
      const readme = buildBackupReadme({
        tenant, tenantId, account, now,
        policyCount: policies.length,
        storedNote: 'This backup was also saved to the app\'s internal backup list when it was\ncreated (shown with a "Manual" badge).',
      })
  ...
  // One-click full backup to IT Glue: builds the same artifacts as the disk
  // export, zips them, then creates a document in the chosen organisation's
  // Documents section and attaches the zip to it.
  // The same snapshot is also registered in the internal backup list.
  ipcMain.handle('backup:uploadToItGlue', async (_, { policies: pols, tenantId, account, orgId, orgName, nameMap = {} } = {}) => {
    try {
      const policies = (Array.isArray(pols) ? pols : []).filter(Boolean)
      if (policies.length === 0) {
        return { success: false, error: 'No policies to back up — load policies from a tenant first' }
      ...
      if (!orgId) {
        return { success: false, error: 'Choose an IT Glue organisation first' }
      }

      const tenant = await resolveTenantName({ account, tenantId })
      const now = new Date()
      const ts = now.toISOString()
      const stamp = ts.slice(0, 19).replace(/[:T]/g, '-')
      const baseName = `PolicyBackup_${backupSafeName(tenant)}_${stamp}`
      ...
      const readme = buildBackupReadme({
        tenant, tenantId, account, now,
        policyCount: policies.length,
        storedNote: `This backup was uploaded to IT Glue (organisation: ${orgName || orgId}) and\nalso saved to the app's internal backup list (shown with an "IT Glue" badge).`,
      })

      const zip = new JSZip()
      zip.file('policies-backup.json', JSON.stringify(manifest, null, 2))
      const policiesFolder = zip.folder('policies')
      for (const entry of buildPolicyFileEntries(policies)) {
        policiesFolder.file(entry.filename, entry.content)
      }
      ...
      const documentName = `M365 Policy Backup - ${tenant} - ${ts.slice(0, 10)} ${ts.slice(11, 16)} UTC`
      const upload = await itGlue.uploadBackup({
        orgId,
        documentName,
      ...
      // Register the same snapshot in the internal (restorable) backup list
      let internal = null
      try {
        internal = writeInternalBackup({ policies, tenantId, account, trigger: 'itglue' })
      } catch (err) {
        logger.warn('backup:uploadToItGlue internal backup failed: ' + err.message)
      }

      logger.info(`backup:uploadToItGlue count=${policies.length} org=${orgId} doc=${upload.documentId} zipBytes=${zipBuffer.length}`)
      return {
        success: true,
        ...
        timestamp: internal?.timestamp || ts,
      }
    } catch (err) {
      logger.error('backup:uploadToItGlue error: ' + err.message)
      return { success: false, error: err.message }
    }
  })
